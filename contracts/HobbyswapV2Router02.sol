...
import './interfaces/IHobbyswapV2Router02.sol';
import './interfaces/IWETH.sol';
import './libraries/SafeMath.sol';
import './libraries/TransferHelper.sol';
import './libraries/HobbyswapV2Library.sol';

contract HobbyswapV2Router02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IHobbyswapV2Pair(HobbyswapV2Library.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HobbyswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = HobbyswapV2Library.sortTokens(input, output);
            IHobbyswapV2Pair pair = IHobbyswapV2Pair(HobbyswapV2Library.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = HobbyswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HobbyswapV2Library.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'HobbyswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(HobbyswapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'HobbyswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return HobbyswapV2Library.getAmountIn(amountOut, reserveIn, reserveOut);
    }
